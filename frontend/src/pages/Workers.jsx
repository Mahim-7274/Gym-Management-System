...
const Workers = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            showMessage("❌ Error adding worker"); 
        }
    };
    ...
    // FEATURE 7 LOGIC: Filter the list based on Name, Phone, or ID securely
    const filteredWorkers = workers.filter(worker => {
        const term = searchTerm.toLowerCase();
        const wName = worker.name ? worker.name.toLowerCase() : "";
        const wPhone = worker.phone ? String(worker.phone).toLowerCase() : "";
        const wId = worker._id ? String(worker._id).toLowerCase() : "";
        
        return wName.includes(term) || wPhone.includes(term) || wId.includes(term);
    });
                    ...
                    filteredWorkers.map(worker => (
                        <div key={worker._id} style={cardStyle}>
                            <div>
                                ...
                                <p style={{ margin: 0, color: '#00d4ff' }}>{worker.phone || 'No Phone'}</p>
                            </div>
